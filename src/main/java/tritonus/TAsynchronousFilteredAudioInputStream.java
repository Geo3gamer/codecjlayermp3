...
/**
 * Base class for asynchronous converters. This class serves as base class for converters that do not have a fixed ratio between the size of a block of input data and the size of a block of output
 * data. These types of converters therefore need an internal buffer, which is realized in this class.
 *
...
public abstract class TAsynchronousFilteredAudioInputStream extends TAudioInputStream implements TCircularBuffer.Trigger {
	 ...
	 *
	 * @param lLength length of this stream in frames. May be AudioSystem.NOT_SPECIFIED.
	 */
	...
	/**
	 * Check if writing more data to the circular buffer is recommended. This checks the available write space in the circular buffer against the minimum available property. If the available write space
	 * is greater than the minimum available property, more writing is encouraged, so this method returns true. Note that this is only a hint to subclasses. However, it is an important hint.
	 *
	 * @return true if more writing to the circular buffer is recommended. Otherwise, false is returned.
	 */
