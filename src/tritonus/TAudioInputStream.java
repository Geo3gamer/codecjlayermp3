...
/**
 * AudioInputStream base class. This class implements "dynamic" properties. "Dynamic" properties are properties that may change during the life time of the objects. This is typically used to pass
 * information like the current frame number, volume of subbands and similar values. "Dynamic" properties are different from properties in AudioFormat and AudioFileFormat, which are considered
 * "static", as they aren't allowed to change after creating of the object, thereby maintaining the immutable character of these classes.
 */
public class TAudioInputStream extends AudioInputStream {
	...
	/**
	 * Constructor with properties. The passed properties map is not copied. This allows subclasses to change values in the map after creation, and the changes are reflected in the map the
	 * application program can obtain.
	 */
	...
	private void initMaps(Map<String, Object> properties) {
		// Here, we make a shallow copy of the map. It's unclear if this is sufficient (of if a deep copy should be made).
