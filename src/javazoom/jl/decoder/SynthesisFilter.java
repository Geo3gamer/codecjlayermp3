...
final class SynthesisFilter {
	...
	/**
	 * Quality value for controlling CPU usage/quality trade-off.
	 */
	/*
	private int quality;
	private int v_inc;
	...
	*/
	...
	private void compute_pcm_samples(Obuffer buffer) {
		...
		/*
		// Considering putting in quality control for low-spec CPUs, but the performance gain (about 10-15%) did not justify the considerable drop in audio quality.
		switch (inc) {
			case 16:
	...
	public void calculate_pcm_samples(Obuffer buffer) {
		...
		*/
		...
		// This may not be necessary. The Layer III decoder always outputs 32 subband samples, but I haven't checked layer I & II.
		for (int p = 0; p < 32; p++) {
			samples[p] = 0.0f;
		}
	}
	...
	// These values are not in the same order as in Annex 3-B.3 of the ISO/IEC DIS 11172-3.
	...
	// The original data for d[]. This data is loaded from a file to reduce the overall package size and to improve performance.
	/*
